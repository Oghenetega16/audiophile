      ...
      <div className="fixed top-[90px] md:top-[114px] left-1/2 -translate-x-1/2 md:right-10 md:left-auto md:translate-x-0 w-[calc(100%-48px)] max-w-[377px] bg-white z-50 rounded-lg shadow-2xl p-8 animate-slideIn">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {items.length > 0 && (
            <button
              onClick={clearCart}
              className="text-black/50 hover:text-[#D87D4A] underline text-[15px] transition-colors"
            >
              Remove all
            </button>
          )}
        ...
        {items.length === 0 ? (
          ...
          <>
              ...
              {items.map((item) => (
                <div key={item.product.id} className="flex items-center gap-4">
                  ...
                  <div className="bg-[#F1F1F1] flex items-center h-8">
                    ...
                    <span className="px-3 font-bold text-[13px] tracking-[1px] min-w-5 text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() =>
            ...
            <div className="flex items-center justify-between mb-6">
              <span className="text-black/50 uppercase text-[15px] font-medium">
                Total
              </span>
            ...
            <Link
              href="/checkout"
              onClick={closeCart}
              className="block w-full bg-[#D87D4A] hover:bg-[#FBAF85] text-white text-center uppercase text-[13px] font-bold tracking-[1px] py-4 transition-colors"
            >
              Checkout
            </Link>
          </>
